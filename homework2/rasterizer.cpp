...
static bool insideTriangle(int x, int y, const Vector3f *_v) {
  ...
  float z_AB = AB.cross(AP).z();
  float z_BC = BC.cross(BP).z();
  float z_CA = CA.cross(CP).z();

  return (z_AB > 0 && z_BC > 0 && z_CA > 0) ||
         (z_AB < 0 && z_BC < 0 && z_CA < 0);
}
...
getTriangleBoundingBox(const Triangle &t) {
  float left = t.v[0].x();
  float right = t.v[0].x();
  float top = t.v[0].y();
  float bottom = t.v[0].y();

  for (int i = 1; i < 3; ++i) {
    left = t.v[i].x() < left ? t.v[i].x() : left;
    right = t.v[i].x() > right ? t.v[i].x() : right;
    top = t.v[i].y() > top ? t.v[i].y() : top;
    bottom = t.v[i].y() < bottom ? t.v[i].y() : bottom;
  }
...
void rst::rasterizer::rasterize_triangle(const Triangle &t) {
  ...
  auto [left, top, right, bottom] = getTriangleBoundingBox(t);

  for (int x = (int)left; x < right; x++) {
    for (int y = (int)bottom; y < top; y++) {
      // 2. Iterate through the pixel and find if the current pixel is inside
      // the triangle. If so, use the following code to get the interpolated z
      // value.
      ...
      // 3. set the current pixel (use the set_pixel function) to the color of
